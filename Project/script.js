const app = function(){
    ...
    function init(){
        ...
        game.bet = 0;
        buildGameBoard();
        turnOff(game.btnHit);//ca butoanele hit si stand sa nu fie apasate inainte sau dupa
    ...
    function addClicker(){
        //functie pentru a putea apasa pe butoane
        //face legatura intre butoane si functii
    ...
    function updateCount(){
        ...
        if(dealer == 21 && game.dealerHand.length == 2){
           game.status.textContent = "Deale Got Blackjack";
            gameEnd();
            findWinner();
           }
    }
    ...
    function takeCard(hand,ele,h){
        ...
        let temp = game.deck.shift(); //scoatem prima carte din pachet(cartea dealerului)
        console.log(temp);
        hand.push(temp);
        console.log(game);
        showCard(temp, ele);
    ...
    //CREEAZA CARTILE(STYLE)
    function showCard(card, el){
        if( card != undefined){
        //el.innerHTML = card.rank + "&" + card.suit + ";";
        el.style.backgroundColor = "white";
        let div = document.createElement("div");   div.classList.add('card');
    ...
    function buildDeck(){
        ...
        for( let i=0;i<suits.length;i++){
            for(let j=0;j<ranks.length;j++){
                ...
                let card = {};
                let tempValue = isNaN(ranks[j]) ? 10: ranks[j];//isNaN- functie it not a number
                //caz special pentru A 
                //poate fi 1 ori 11
     ...
     //functie pentru a ameteca cartile
